...
<template>
  <view
    class="_tab-box"
    :style="{ fontSize: defaultConfig.fontSize + 'rpx', color: defaultConfig.color }"
  >
    <scroll-view
      id="_scroll"
      :scroll-x="true"
          ...
          <block v-for="(item, index) in tabList" :key="index">
            <view
              ...
              :style="{
                ...
                width: defaultConfig.itemWidth ? defaultConfig.itemWidth + 'rpx' : '',
              }"
...
<script>
// https://ext.dcloud.net.cn/plugin?id=1102 文档地址
export default {
  name: 'LiuyunoTabs',
  props: {
    tabData: {
    ...
    tabToIndex(index) {
      ...
      if (!width) {
        if (this.defaultConfig.itemWidth) {
          ...
          width =
            this.tabList[index][this.defaultConfig.key].length *
            uni.upx2px(this.defaultConfig.fontSize)
        }
